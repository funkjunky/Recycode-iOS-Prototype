...
/*
  ...
  //return [NSString stringWithFormat:@"Scanned!\n\nFormat: %@\n\nContents:\n%@", formatString, result.text];
    ...
    return [NSString stringWithFormat:@"Scanned: %@", result.text];
 
 */
    
    if ([result.text isEqualToString:@"180854000101"]) {
        return [NSString stringWithFormat:@"%@", @"RedBull"];
    }
    else if ([result.text isEqualToString:@"067311016243"]) {
        return [NSString stringWithFormat:@"%@", @"OASIS Juice"];
    }
    else {
...
{
    self.decodedLabel.text = @"scaning . . .";}
